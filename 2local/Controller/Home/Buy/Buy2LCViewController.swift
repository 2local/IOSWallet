...
class Buy2LCViewController: BaseVC, UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    guard let cell = tableView.dequeueReusableCell(withIdentifier: "paymentMethod") as? PaymentMethodTableViewCell else { return UITableViewCell() }
    cell.titleLabel.text = paymentMethods[indexPath.row]
    tableView.layer.borderColor = UIColor.f6f6f9.cgColor
    if paymentMethods[indexPath.row] == paymentMethodBTN.titleLabel?.text {
      cell.backgroundColor = UIColor.f6f6f9
      cell.titleLabel.textColor = UIColor.color303030
    } else {
      ...
      cell.titleLabel.textColor = UIColor.color606060
    }
  ...
  fileprivate func addOrder() {
    /*
    APIManager.shared.addOrder(userId: DataProvider.shared.user!.id ?? -1,
                               quantity: finalCost,
                               currency: DataProvider.shared.defaultEx!,
                               amount: finalCost,
                               status: "open",
                               date: Date().description,
                               paymentType: self.paymentMethodBTN.titleLabel!.text!.lowercased(),
                               l2lQuantity: l2lQuantity) { (data, response, _) in
    ...
    if self.amountTXF.text != "" {
      ...
      if finalCost >= 2.50 {
        ...
        if self.paymentMethodBTN.titleLabel?.text == "PayPal" {
          APIManager.shared.mollie(userId: DataProvider.shared.user!.id ?? -1,
                                   quantity: finalCost,
                                   currency: DataProvider.shared.defaultEx!,
                                   amount: finalCost,
                                   paymentType: "paypal",
                                   l2lQuantity: l2lQuantity) { (data, _, error) in
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    if segue.identifier == "goToBtcMode", let sender = sender as? Double {

      if let navVC = segue.destination as? UINavigationController {
        if let destVC = navVC.children.first as? Buy2LCReceiptViewController {
          destVC.price = "\(sender)"
          var btcAmount = (sender) / ((DataProvider.shared.exchangeRate?.bitcoin?.defaultExchangeRate)! as NSDecimalNumber).doubleValue

          if self.paymentMethodBTN.titleLabel?.text == "Stellar" {
            btcAmount = (sender) / ((DataProvider.shared.exchangeRate?.stellar?.defaultExchangeRate)! as NSDecimalNumber).doubleValue
            destVC.type = "XLM"
            destVC.walletNumber = DataProvider.shared.stellarWalletNumber
          } else if self.paymentMethodBTN.titleLabel?.text == "Ethereum" {
            btcAmount = (sender) / ((DataProvider.shared.exchangeRate?.ethereum?.defaultExchangeRate)! as NSDecimalNumber).doubleValue
            destVC.type = "ETH"
            destVC.walletNumber = DataProvider.shared.etheriumWalletNumber
          } else {
            btcAmount = (sender) / ((DataProvider.shared.exchangeRate?.bitcoin?.defaultExchangeRate)! as NSDecimalNumber).doubleValue
            destVC.type = "BTC"
