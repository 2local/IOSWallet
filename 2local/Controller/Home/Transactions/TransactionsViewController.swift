...
class TransactionsViewController: BaseVC {
  ...
  @IBOutlet var chartsSegmentedControl: SegmentedControl! {
    didSet {
      self.chartsSegmentedControl.setTitles([NSAttributedString(string: "Income",
                                                                attributes: attributes),
                                             NSAttributedString(string: "Expenses",
                                                                attributes: attributes)],
                                            selectedTitles: [NSAttributedString(string: "Income",
      ...
      self.chartsSegmentedControl.selectionIndicatorColor = .flamenco
      self.chartsSegmentedControl.selectionIndicatorHeight = 1
      self.chartsSegmentedControl.selectionIndicatorEdgeInsets = UIEdgeInsets(top: 0,
                                                                              left: 17,
                                                                              bottom: 0,
                                                                              right: 17)
    }
    ...
    didSet {
      self.transactionSegmentedControl.setTitles(
        [NSAttributedString(string: "All", attributes: attributes)
         ...
         ,NSAttributedString(string: "Received", attributes: attributes), NSAttributedString(string: "Sent", attributes: attributes)], selectedTitles:
          [NSAttributedString(string: "All", attributes: selectedAttributes)
           ...
           ,NSAttributedString(string: "Received", attributes: selectedAttributes), NSAttributedString(string: "Sent", attributes: selectedAttributes)])
  ...
  @IBOutlet var lineChart: LineChartView! {
    didSet {
      ...
      lineChart.xAxis.labelFont = .TLFont(weight: .regular,
                                          size: 10)
      lineChart.xAxis.labelTextColor = .blueHaze
      lineChart.xAxis.axisLineColor = .solitude
  ...
  // MARK: - properties
  var index = 0
  let attributes: [NSAttributedString.Key: Any] = [NSAttributedString.Key.foregroundColor: UIColor.logan, NSAttributedString.Key.font: UIFont.TLFont(weight: .regular, size: 13)]
  let selectedAttributes: [NSAttributedString.Key: Any] = [NSAttributedString.Key.foregroundColor: UIColor.flamenco, NSAttributedString.Key.font: UIFont.TLFont(weight: .medium, size: 13)]
...
extension TransactionsViewController: ChartViewDelegate {
  ...
  func setupLineChart(source: String, firstSetup: Bool) {
    ...
    let allTransactions = self.allTransactions
      .filter { source == "in" ? ($0.source == source || $0.source == "Purchase" || $0.to?.lowercased() == $0.wallet?.address.lowercased()) : $0.source == source }
      .filter {($0.status == "paid" || $0.status == "Complete" || $0.status == "completed" || $0.from?.lowercased() == $0.wallet?.address.lowercased()) }
    ...
    lineChartDataSet.valueFont = .TLFont(weight: .medium,
                                         size: 10)
    lineChartDataSet.valueColors = [.blueHaze]
    lineChartDataSet.lineWidth = 1.5
    lineChartDataSet.circleRadius = 5
    lineChartDataSet.circleHoleRadius = 0
    lineChartDataSet.colors = [.lightSlateBlue]
    lineChartDataSet.circleColors = [.mediumSlateBlue]
    lineChartDataSet.circleHoleColor = .mediumSlateBlue
