...
class CreatePasswordVC: BaseVC {
    ...
    fileprivate func setupView() {
        ...
        ([passwordTextField, confirmPasswordTextField] as! [SkyFloatingLabelTextField]).forEach { (textField) in
        ...
        ([titleLabel, descriptionLabel, biometricTitleLabel, acceptionTitleLabel] as! [UILabel]).forEach { (label) in
            label.textColor = ._707070
        }
        
        createPasswordButton.backgroundColor = ._EF8749
    ...
    fileprivate func updateAgreementStatus() {
        ...
        if agreement {
            ...
            acceptionTitleLabel.textColor = ._303030
            createPasswordButton.backgroundColor = ._EF8749
            createPasswordButton.isEnabled = true
        } else {
            acceptionButton.setImage(UIImage(named: "checkEmpty"), for: .normal)
            acceptionTitleLabel.textColor = ._707070
            DispatchQueue.main.async {
                self.createPasswordButton.backgroundColor = UIColor._EF8749.withAlphaComponent(0.5)
            }
