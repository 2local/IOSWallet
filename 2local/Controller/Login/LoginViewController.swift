...
class LoginViewController: BaseVC, TwoVerificationDelegate {
  ...
  let hidePasswordImage = UIImage(named: "eyeHide")?.tint(with: .color9796AE)
  let showPasswordImage = UIImage(named: "eyeFill")?.tint(with: .color9796AE)
  ...
  func login () {
    if emailTXF.text != "" && passwordTXF.text != "" {
      ...
      APIManager.shared.login(email: emailTXF.text!, password: passwordTXF.text!) { (data, response, _) in
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    if segue.identifier == "goTo2FA" {
      if let destVC = segue.destination as? LoginTFAViewController {
