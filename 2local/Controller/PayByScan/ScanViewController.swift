  ...
  func switchCamera() {
    if let session = self.cameraView.captureSession {
      if session.inputs.count != 0 {
        ...
        if let currentCameraInput = currentCameraInput as? AVCaptureDeviceInput,
           currentCameraInput.device.position == .back {
          UIView.transition(with: self.cameraView,
                            duration: 0.5,
      ...
      do {
        ...
        if device.torchMode == .on {
          ...
          UIView.animate(withDuration: 0.2) {
            sender.backgroundColor = .mortar
          }
        } else if device.torchMode == .off {
          device.torchMode = .on
          UIView.animate(withDuration: 0.2) {
            sender.backgroundColor = .flamenco
          }
        }
