...
extension Web3Service {
    ...
    func import2LCBy(privateKey: String, completion: @escaping (String?) -> ()) throws {
        do {
            guard let privateKeyData = Data.fromHex(privateKey) else {
                KVNProgress.showError(withStatus: "Private key is not valid!")
                return
            }
            let ks = try EthereumKeystoreV3(privateKey: privateKeyData, password: password ,aesMode: aesMode)
    ...
    func sendBEP20Token(walletAddress: String, receiverAddress: String, tokenAmount: String, gasPrice: BigUInt, gasLimit: BigUInt, completionHandler: @escaping (String) -> Void) {
        do {
            ...
            let token = ERC20(web3: Web3Service.shared.web3Manager, provider: Web3Service.shared.isMainnet() ? Web3.InfuraMainnetWeb3().provider : Web3.InfuraRopstenWeb3().provider, address: EthereumAddress(tokenContractAddress)!)
            ...
            let contratInstance = try contract.method("transfer", parameters: [receviverBnbAddress as Any, amount as Any] as [AnyObject], extraData: Data(), transactionOptions: options)?.send(password: "", transactionOptions: options)
    ...
    func getBSCGasPrice(_ completion: @escaping (BigUInt?) -> Void) {
        APIManager.shared.getBSCGasPrice { (data, response, error) in
            do {
