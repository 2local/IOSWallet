...
extension Web3Service {
    ...
    static func getPrivateKey() -> String? {
        ...
        let pkData = try! keystoreManager.UNSAFE_getPrivateKeyData(password: wallet.password, account: ethereumAddress).toHexString()
        return pkData
    }
    ...
    func getETHGasPrice(_ completionHandler: @escaping (GasPriceModel?) -> Void) {
        APIManager.shared.getGasPrice { (data, response, error) in
    ...
    func getETHTransactionHistory(by address: String, completionHandler: @escaping ([TransactionHistoryModel]?) -> Void) {
