...
class TLNavigationController: UINavigationController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationBar.titleTextAttributes = [.font: UIFont.TLFont(weight: .medium, size: 16), .foregroundColor: UIColor.color606060]
        self.navigationBar.largeTitleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.color606060, NSAttributedString.Key.font: UIFont.TLFont(weight: .medium, size: 24)]
        self.navigationBar.tintColor = .color606060
        setupStyle()
    }
    ...
    func setupStyle() {
        if isTransparentStyled {
            ...
            self.navigationBar.setBackgroundImage(UIImage(), for: .default)
            self.navigationBar.shadowImage = hasShadow ? UIColor.color606060.withAlphaComponent(0.2).as1ptImage() : UIImage()
    ...
    func isTransparentNavigationBarWithShadow(_ isTrue: Bool = true) {
        let navBarBackGround: UIImage = isTrue ? UIImage() : UIColor.color606060.withAlphaComponent(0.2).as1ptImage()
